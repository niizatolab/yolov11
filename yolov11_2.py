...
try:
    while True:
        ...
        if results and len(results) > 0 and results[0].boxes is not None and results[0].boxes.xyxy is not None:
            ...
            track_ids = results[0].boxes.id.cpu().numpy() if results[0].boxes.id is not None else [-1] * len(boxes)
            classes = results[0].boxes.cls.cpu().numpy() if results[0].boxes.cls is not None else [-1] * len(boxes)

            print(f"検出数: {len(boxes)}")  # デバッグ用ログ

            for box, track_id, cls in zip(boxes, track_ids, classes):
                x1, y1, x2, y2 = map(int, box[:4])
                label = f"ID: {int(track_id)} Class: {int(cls)}"
                print(f"検出: {label} ({x1}, {y1}, {x2}, {y2})")  # 検出結果をログに出力
